...
const Home = () => {
  ...
  const handleHover = (e) => {
    if (activeTab !== e.target) {
      e.target.classList.add("nav-gradient");
    }
  };
  const handleLeave = (e) => {
    if (activeTab !== e.target) {
      e.target.classList.remove("nav-gradient");
    }
  };
  ...
  const handleActiveTab = (e) => {
    ...
    e.target.style.backgroundColor = "#f0ebf4";
              ...
              >
                <ul className="flex flex-col mt-4 font-medium space-y-3">
                  <li>
                    <a
                      href="#home"
                      className=" block py-2 pr-4 pl-3 text-gray-400 rounded text-center  "
                      aria-current="page"
